...
class Preprocessor:
    ...
    def build_from_path(self):
        ...
        with open(join(self.out_dir, "stats.json"), "w") as f:
            stats = {
                "pitch": [
                    # float(pitch_min),
                    # float(pitch_max),
    ...
    def get_alignment(self, tier):
        ...
        for t in tier._objects:
            ...
            if p not in sil_phones:
                ...
                p = self.duration_symbol_setter(e - s)
                phones.append(p)
    ...
    def duration_symbol_setter(self, duration):
        if duration <= 0.1:
            symbol = 'd1'
        elif 0.1 < duration <= 0.3:
            symbol = 'd2'
        elif 0.3 < duration <= 0.6:
            symbol = 'd3'
        elif 0.6 < duration <=1.0:
            symbol = 'd4'
        elif 1.0 < duration:
            symbol = 'd5'
