...
from tts.fastspeech2.fastspeech2_JCU2 import FastSpeech2
from tts.discriminator.jcu_discriminator import JCU
from tts.discriminator.loss import AdversarialLoss
from nets.intensity_extractor.model import RankModel
...
def resume(
    ...
    if train:
        ...
        with open('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/intensity.yaml', 'r') as f:
            config = yaml.safe_load(f)
        model.intensity_model = RankModel(config).to(device).eval()
        model.load_state_dict(states["model"])
        logging.info(f"The training was resumed using {checkpoint}")
