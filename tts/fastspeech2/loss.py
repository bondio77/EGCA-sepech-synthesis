...
class FastSpeech2Loss(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_masking:
            ...
            # g_pitch_var = g_pitch_var.unsqueeze(2).masked_select(pitch_masks)
            # pitch_var = pitch_var.masked_select(pitch_masks)
            # g_duration_var = g_duration_var.unsqueeze(2).masked_select(pitch_masks)
            # duration_var = duration_var.masked_select(pitch_masks)
            # g_pitch_diff = g_pitch_diff.unsqueeze(2).masked_select(pitch_masks)
            # pitch_diff = pitch_diff.masked_select(pitch_masks)
        ...
        # duration_var_loss = self.mse_criterion(g_duration_var, duration_var)
        # pitch_diff_loss = self.mse_criterion(g_pitch_diff, pitch_diff)
        ...
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
            ...
            return l1_loss, duration_loss, pitch_loss, energy_loss, pitch_max_loss #, duration_var_loss
