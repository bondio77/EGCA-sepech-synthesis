...
class IntensityExtractor(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_emotions):
        ...
        self.asp_bn = BatchNorm1d(input_size=hidden_dim)
    ...
    def forward(self, x, emotion_class):
        ...
        frame_x0 = self.feature_aggregate(x)
        hidden_attention = self.asp(frame_x0)
        # temporal_attention = self.asp_bn(hidden_attention)
        # h_mix = temporal_attention.mean(dim=2)
        h_mix = hidden_attention.mean(dim=2)
        # mean_x = frame_x0.mean(dim=-1)

        return h_mix, hidden_attention
