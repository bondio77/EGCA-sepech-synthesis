...
class RankModel(nn.Module):
    def __init__(self, configs):
        ...
        self.intensity_extractor = IntensityExtractor(82, configs['hidden_dim'], configs['num_layers'],
                                                      configs['num_emotions'])
        self.projector = nn.Sequential(
            nn.Linear(configs['hidden_dim'], configs['hidden_dim'] // 2),
            nn.GELU(),
            nn.LayerNorm(configs['hidden_dim'] // 2),
            nn.Linear(configs['hidden_dim'] // 2, 1)
        )
        ...
        #     nn.GELU(),
        #     nn.LayerNorm(configs['hidden_dim'] // 2),
        #     nn.Linear(configs['hidden_dim'] // 2, configs['num_emotions'])
        # )
