...
from speechbrain.speechbrain.nnet.CNN import Conv1d as _Conv1d
from speechbrain.speechbrain.nnet.normalization import BatchNorm1d as _BatchNorm1d
...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        # Apply layers
        attn_0 = self.conv(self.tanh(self.tdnn(attn)))

        # Filter out zero-paddings
        # attn = attn.masked_fill(mask == 0, float("-inf"))

        attn = F.softmax(attn_0, dim=2)
        mean, std = _compute_statistics(x, attn)
        # Append mean and std of the batch
        pooled_stats = torch.cat((mean, std), dim=1)
        rf_pool = self.fc(pooled_stats)
        # weight = self.sigmoid(rf_pool).unsqueeze(2)
        weight = self.gelu(rf_pool).unsqueeze(2)
